...
@Component({
  selector: 'app-tableprice3',
  templateUrl: './tableprice3.component.html',
  styleUrls: ['./tableprice3.component.scss']
})
export class Tableprice3Component implements OnInit {
    ...
    paypal
      .Buttons({
        ...
        onApprove: async (data, actions) => {
          ...
          this.showSuccessToaster('You have bought 30 SLDR');
          this.updateGas();
        },
        onError: err => {
  ...
  * Dedicated method to update the User Gas after payment
  ...
  updateGas(): void {
    this.userService.getUser(this.currentUser.email).subscribe(data => {
      this.user = data;
      this.user.userGaz = this.user.userGaz + 30;
